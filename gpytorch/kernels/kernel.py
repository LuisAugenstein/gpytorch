            ...
            if settings.lazily_evaluate_kernels.on() and self._lazily_evaluate:
                ...
                if len(named_parameters_parent_modules_and_batch_dimensions):
                    ...
                    res = KernelLinearOperator(
                        ...
                        module_params=module_params,  # kwarg for _kernel_linear_operator_covar_func
                        non_param_kwargs=dict(last_dim_is_batch=last_dim_is_batch, **kwargs),  # kwarg for forward
                        **params,
                    )
                else:
                    res = KernelLinearOperator(
